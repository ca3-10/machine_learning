class Matrix:
    ...
    def __add__(self, add_matrix):
        ...
        for i in range(self.num_rows):
            ...
            for j in range(self.num_cols):
                added_matrix[i].append(self.elements[i][j] + add_matrix.elements[i][j])
        return Matrix(added_matrix)


    def __sub__(self, sub_matrix):
        subtracted_matrix = []
        for i in range(self.num_rows):
            subtracted_matrix.append([])
            for j in range(self.num_cols):
                subtracted_matrix[i].append(self.elements[i][j] - sub_matrix.elements[i][j])
    ...
    def __matmul__(self,times_matrix):
        multi_matrix = []
        for i in range(self.num_rows):
            multi_matrix.append([])
            for j in range(len(times_matrix.elements[0])):
                multi_matrix[i].append(0)
                for k in range(len(times_matrix.elements)):
                    multi_matrix[i][j] += (self.elements[i][k] * times_matrix.elements[k][j])
